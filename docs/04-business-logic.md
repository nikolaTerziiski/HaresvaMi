...
row is reported as an empty category and stays visible in category lists, but it
does not count as a ready product or dish preview. A row with a dish name must
also include a BGN price; otherwise the save is blocked and the owner sees a
category + dish message, for example: `В категория „Салати“ за ястие „Кебапче“ не сте попълнили цена.`
