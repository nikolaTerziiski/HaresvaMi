    ...
    <div className="sticky bottom-5 z-[60] mx-9 mt-8 max-md:mx-5">
      ...
      {hasOnlyHiddenChanges ? (
        <div className="mb-1.5 flex items-center gap-1 px-2 text-[12px] text-[var(--ink-mute)]">
          <span>{t("hiddenChangesHint")} ·</span>
          <button
            type="button"
            onClick={onClearCategoryFilter}
            className="underline decoration-[var(--accent)] text-[var(--accent)] hover:opacity-80 transition-opacity"
          >
            {t("showAll")}
          </button>
        </div>
      ...
      <div className="flex items-center gap-4 rounded-xl bg-[var(--ink)] px-5 py-3.5 text-[var(--paper)] shadow-[0_20px_40px_-10px_rgba(26,21,18,0.4)] max-md:flex-wrap">
