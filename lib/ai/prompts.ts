...
Задача: Извлечи САМО храна и десерти от всички файлове и ги върни като JSON.

КРИТИЧНО ПРАВИЛО — ПРОПУСКАЙ НАПИТКИ:
  НЕ извличай напитки от никакъв вид. Пропусни:
  - алкохол (вино, бира, ракия, водка, уиски, ликьори, коктейли)
  - безалкохолни (сокове, газирани напитки, кола, фанта, домашна лимонада, айрян)
  - топли напитки (кафе, чай, капучино, еспресо)
  - вода (минерална, газирана, негазирана)
  Дори ако са в менюто с цена, ПРОПУСНИ ги. Извличай само ястия и десерти.

ЗАДЪЛЖИТЕЛНИ КАТЕГОРИИ ЗА ХРАНА (използвай САМО тях):
  Салати, Супи, Предястия, Основни, Скара, Гарнитури, Десерти

Ако не можеш да определиш категорията на дадено ястие, използвай "Некласифицирано". НЕ измисляй нови категории.

За всяко ястие включи полето source_file_name — точното име на файла, от който е взето.

Цените са в лева (лв, BGN) или евро (€). Върни само числовата стойност без валута.
...
4. ПРОПУСКАЙ всички напитки безусловно (виж критичното правило по-горе).
`.trim();
}
